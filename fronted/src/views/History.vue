<template>
  <div class="list-page">
    <van-nav-bar title="浏览历史" left-text="返回" left-arrow @click-left="onClickLeft" right-text="清空" @click-right="onClickClear" fixed />
    <div class="list-content" v-if="historyStore.getHistory.length">
      <div class="list-item" v-for="item in historyStore.getHistory" :key="item.id">
          ...
          <template #title>
            <div class="item-card">
              ...
              <div class="item-info">
                ...
                <div class="item-time">浏览于 {{ item.viewTime }}</div>
              </div>
            </div>
          </template>
        </van-cell>
...
<script setup>
import { onMounted } from 'vue'; import { useRouter } from 'vue-router'; import { useHistoryStore } from '../store/modules/history'; import { showDialog } from 'vant'; import { formatRelativeTime } from '../utils/news';
const router = useRouter(); const historyStore = useHistoryStore();
const onClickLeft = () => router.back();
const goToNewsDetail = (id) => router.push(`/news/detail/${id}`);
const removeHistory = async (id) => { try { const r = await historyStore.removeHistoryApi(id); if (!r.success && !r.isLocal) showDialog({ title: '提示', message: r.message || '删除失败' }) } catch (e) { console.error(e) } };
const confirmDelete = (id) => { showDialog({ title: '提示', message: '确定要删除吗？', showCancelButton: true }).then(a => { if (a === 'confirm') removeHistory(id) }) };
const onClickClear = () => { showDialog({ title: '提示', message: '确定要清空所有浏览历史吗？', showCancelButton: true }).then(async a => { if (a === 'confirm') { try { await historyStore.clearHistoryApi() } catch (e) { console.error(e) } } }) };
onMounted(async () => { try { const r = await historyStore.getHistoryListApi(); if (!r || !r.success) historyStore.loadHistory() } catch (e) { historyStore.loadHistory() } });
</script>
<style scoped>
