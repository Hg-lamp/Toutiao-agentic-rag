  ...
  actions: {
    ...
    async uploadAvatar(file) {
      try {
        ...
        const response = await axios.post(`${apiConfig.baseURL}/api/user/avatar`,
          formData,
          {
            headers: {
              Authorization: this.token,
        ...
        if (response.data && response.data.code === 200) {
          ...
          return { success: true, message: '头像上传成功', avatar: avatarUrl };
        } else {
          return { success: false, message: response.data.message || '头像上传失败' };
        }
      } catch (error) {
        console.error('上传头像失败:', error);
        return { success: false, message: error.response?.data?.detail || '上传头像失败' };
      }
    },
